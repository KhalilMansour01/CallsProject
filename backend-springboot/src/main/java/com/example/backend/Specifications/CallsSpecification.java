    ...
    public static Specification<CallsEntity> searchByMultipleFields(String searchQuery) {
        return (root, query, criteriaBuilder) -> {
            ...
            Predicate cCodePredicate = criteriaBuilder.like(
                    criteriaBuilder.lower(criteriaBuilder.toString(root.get("cCode"))), "%" + lowerSearchQuery + "%");
            Predicate eCodePredicate = criteriaBuilder.like(
                    criteriaBuilder.lower(criteriaBuilder.toString(root.get("eCode"))), "%" + lowerSearchQuery + "%");

            Predicate fCatPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("fCat")),
                    "%" + lowerSearchQuery + "%");
