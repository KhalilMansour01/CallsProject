...
public class CustomerSpecifications {
    ...
    public static Specification<CustomerEntity> hasRegion(String regCode) {
        return (root, query, criteriaBuilder) -> regCode == null || regCode.isEmpty() ? criteriaBuilder.conjunction()
    ...
    public static Specification<CustomerEntity> hasCountry(String cntrCode) {
        return (root, query, criteriaBuilder) -> cntrCode == null || cntrCode.isEmpty() ? criteriaBuilder.conjunction()
    ...
    public static Specification<CustomerEntity> searchByMultipleFields(String searchQuery) {
        return (root, query, criteriaBuilder) -> {
            ...
            Predicate idPredicate = criteriaBuilder.like(
                    criteriaBuilder.lower(criteriaBuilder.toString(root.get("id"))), "%" + lowerSearchQuery + "%");
            Predicate namePredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("custName")),
                    "%" + lowerSearchQuery + "%");
            Predicate tel1Predicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("tel1")),
                    "%" + lowerSearchQuery + "%");
            Predicate tel2Predicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("tel2")),
                    "%" + lowerSearchQuery + "%");
            Predicate tel3Predicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("tel3")),
                    "%" + lowerSearchQuery + "%");
            Predicate faxPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("fax")),
                    "%" + lowerSearchQuery + "%");
            Predicate emailPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("email")),
                    "%" + lowerSearchQuery + "%");
            Predicate contactPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("contact")),
                    "%" + lowerSearchQuery + "%");
    ...
    public static Specification<CustomerEntity> searchByName(String searchQuery) {
        return (root, query, criteriaBuilder) -> {
            if (searchQuery == null || searchQuery.isEmpty()) {
